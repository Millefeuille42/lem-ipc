...
void spawn_behavior(t_app *app) {
	ft_logstr(INFO, "has not spawned\n");
	t_vec first_free = get_first_free(app->shared->map);
	if (errno) {
		errno = 0;
		ft_logstr(ERROR, "could not register position\n");
		return;
	}
	ft_lognbr_in_between(INFO, "registered position ", first_free.x, ",", 0);
	ft_lognbr_in_between(INFO, "", first_free.y, "\n", 1);
